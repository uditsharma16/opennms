  ...
  .controller('PolicyController', ['$scope', '$uibModalInstance', 'RequisitionsService', 'policy', function($scope, $uibModalInstance, RequisitionsService, policy) {
    ...
    */
    $scope.getTemplate = function(parameter) {
            ...
            if (paramCfg.key == parameter.key) { // Checking current parameter
              if (paramCfg.required) {
                if ($scope.isNonEmptyArray(paramCfg.options)) {
                  return 'app/components/admin/requisition/viewspolicy-param.options.html';
                } else {
                  return 'app/components/admin/requisition/viewspolicy-param.string.html';
                }
              }
            }
            if (!paramCfg.required) {
      ...
      return parameter.key ? 'app/components/admin/requisition/viewspolicy-param.fixed.html' : 'app/components/admin/requisition/viewspolicy-param.editable.html';
    };
